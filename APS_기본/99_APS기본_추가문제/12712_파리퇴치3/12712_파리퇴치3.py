...
def kill_fly(arr_fly, di, dj, N, M):  # di, dj 는 델타값을 원소로 하는 리스트
    max_value = 0

    for i in range(N):
        for j in range(N):
            killed_fly = arr_fly[i][j]  # 중간값 더하고 시작
            k = 1
            for d in range(4 * (M - 1)):    # M이 세기였다
                if d >= 4:
                    k = (d // 4) + 1
                d = d % 4
                ni = i + di[d] * k
...
for test_case in range(1, 1+T):
    ...
    # 우 하 좌 상 순서대로 d 돌리고 이 시행이 최대 4*M 번 실행된다.
    # 인덱스 정상일때만 합산

    di = [0, 1, 0, -1]
    dj = [1, 0, -1, 0]

    max_cross = kill_fly(arr_fly, di, dj, N, M)
    ...
    max_X = kill_fly(arr_fly, di, dj, N, M)

    if max_cross > max_X:
        print(f'#{test_case} {max_cross}')
    elif max_cross < max_X:
