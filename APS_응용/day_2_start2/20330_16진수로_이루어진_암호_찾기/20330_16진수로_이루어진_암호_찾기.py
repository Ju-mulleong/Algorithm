...
format(num, 'spelling')
spelling은 'b', 'o', 'x' 각각 2진수, 8진수, 16진수이다.
num을 해당하는 진수에 맞게 변환

format방식 사용하면 0b같은 접두사 없어지는 대신에 0을 변환하면 '0000' 이 아니라 '0'이 된다.
다른 3자리 내의 2진수도 마찬가지

format(num, 0Nd)형태로 하면 맞춰진다

다시 정리, 만약 2진수를 16진수로 바꾸고 싶다면,
먼저 int로 2진수를 10진수로 바꾸고, 
그 수를 다시 format을 사용하여 16진수로 바꾸는 거다.

format(int('0101', 2), '04x')
